...
% x0 = [];
[Q_predicted, x, err, lnP0, psi, Q_IAST, x_IAST, err_IAST, lnP0_IAST, psi_IAST] = RAST_solve(M, S, 'mode', 2, 'tol', 1e-6, 'isotherm', isotherm, 'minlnP', minlnP, 'ads_pot', ads_pot, 'inv_ads_pot', inv_ads_pot, 'EoS', @Margules, 'x0', x0);
gamma = x(:, N+1:end);
...
%% Mode 3
% x0 = [9.57470084341351,4.46925870762674,10.7481878232943,8.50810671171637,10.8548425109636,8.79700397850355,10.7939772728962,8.63279570542132,10.8815465907637,8.86850909073182,10.8047964089486,8.66211129162805,10.8625542204049,8.81768691944788,10.8519348973963,8.78919862797416,10.6819109528877,8.32575011643916,9.51388129990895,4.00586342060703,10.5945409692843,8.07916486198067,9.47798322461679,3.61349941137665,9.82112087045924,5.58830931078986,0.155484271451535,0.0339927870231449,0.00718109052609238,0.0270490598458720,0.00337131900955830,0.0203499003702182,0.00200681638675859,0.0126145645542420,0.0380724293627776,0.174576389109094,0.0463929991500180,0.207426659323684,0.112627257555848,1.10357346686027,0.995678582203219,1.51326511191047];  % MFI-1, 298K
x0 = [];
...
% x0 = [9.21154950381345,5.20333129570762,9.23705889815008,6.17005998011118,9.28682441357335,7.51246176162873,9.72582956708356,9.99492724872032,9.75834759809123,10.1335915791533,9.79193510762030,10.2753931837462,9.80576315163838,10.3332759231074,9.82230207302466,10.4021553971537,9.84448182069593,10.4939379925130,9.83446950436495,10.4525689097347,9.79181675199398,10.2749008004925,58.3348944827058,-14.4139173622543,0.000583663591618939];  % FAU-2, 323K
...
% x0 = [];
[Q_predicted, x, err, lnP0, psi, Q_IAST, x_IAST, err_IAST, lnP0_IAST, psi_IAST] = RAST_solve(M, S, 'mode', 4, 'C_ub', 1e-3, 'tol', 1e-6, 'isotherm', isotherm, 'minlnP', minlnP, 'ads_pot', ads_pot, 'inv_ads_pot', inv_ads_pot, 'EoS', @Margules_ads, 'x0', x0, 'N_EoS_param', 3, 'EoS_deriv', @Margules_ads_deriv);
coeff=x(ndata*N+1:end);

%% Mode 5
% x0 = [3.89539206734144,1.15707216080434,0.00550909773523617];  % MFI-1, 298K
x0 = [4.685046915868567  -1.543292562885986  85.980231912381001];  % FAU-2, 323K
% x0 = [];
[Q_predicted, x, err, lnP0, psi, Q_IAST, x_IAST, err_IAST, lnP0_IAST, psi_IAST] = RAST_solve(M, S, 'mode', 5, 'C_ub', 1e-2, 'tol', 1e-6, 'isotherm', isotherm, 'minlnP', minlnP, 'ads_pot', ads_pot, 'inv_ads_pot', inv_ads_pot, 'EoS', @Margules_ads, 'x0', x0, 'N_EoS_param', 3, 'EoS_deriv', @Margules_ads_deriv);
coeff = x;

%% RAST fitting with fitted EoS
x0 = [z(:,1:end-1), lnP0];
[err, Q_predicted, x, err_IAST, lnP0_IAST, psi_IAST] = RAST_func_IAST_solve(coeff, isotherm, M, @Margules_ads, 'mode', 1, 'tol', 1e-6, 'minlnP', minlnP, 'maxlnP', maxlnP, 'ads_pot', ads_pot, 'inv_ads_pot', inv_ads_pot, 'EoS_deriv', @Margules_ads_deriv, 'x0', x0, 'options', optimset('Display', 'iter'));
