...
% x0 = [];
[Q_predicted, x, err, lnP0, psi, Q_IAST, x_IAST, err_IAST, lnP0_IAST, psi_IAST] = RAST_solve(M, S, @Margules, [], 1, x0);
gamma = x(:, N+1:end);
...
x0 = [];
[Q_predicted, x, err, lnP0, psi, Q_IAST, x_IAST, err_IAST, lnP0_IAST, psi_IAST] = RAST_solve(M, S, @Margules, [], 3, x0, 2);
...
% x0 = [];
[Q_predicted, x, err, lnP0, psi, Q_IAST, x_IAST, err_IAST, lnP0_IAST, psi_IAST] = RAST_solve(M, S, @Margules_ads, [], 4, x0, 3, @Margules_ads_deriv);
...
% x0 = [];
[Q_predicted, x, err, lnP0, psi, Q_IAST, x_IAST, err_IAST, lnP0_IAST, psi_IAST] = RAST_solve(M, S, @Margules_ads, [], 5, x0, 3, @Margules_ads_deriv);
coeff = x;

%% RAST fitting with fitted EoS
[isotherm, minlnP] = fit_isotherm(S);
x0 = [z(:,1:end-1), lnP0];
[err, Q_predicted, x, err_IAST, lnP0_IAST, psi_IAST] = RAST_func_IAST_solve(coeff, isotherm, minlnP, M, @Margules_ads, @Margules_ads_deriv, 2, x0 ,[], optimset('Display', 'iter'));
